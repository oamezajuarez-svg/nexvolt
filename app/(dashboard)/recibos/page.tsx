...
import { getInvoicesByOrg } from "@/lib/actions/invoices";
...
export default function RecibosPage() {
  ...
  const [savedInvoices, setSavedInvoices] = useState<
    { id: string; period: string; total_cost: number | null; total_kwh: number | null }[]
  >([]);
  const [dragOver, setDragOver] = useState(false);

  // Load org and existing invoices on mount
  const loadData = useCallback(async () => {
    setOrgLoading(true);
    const orgResult = await getMyOrganization();
    if (orgResult.data) {
      setOrgId(orgResult.data.id);
      const invResult = await getInvoicesByOrg(orgResult.data.id);
      if (invResult.data) {
        setSavedInvoices(
  ...
  const handleSave = async (upload: UploadedInvoice) => {
    if (!orgId || !upload.extracted) return;
  ...
  if (orgLoading) {
    return (
      <div className="p-8 flex items-center justify-center min-h-[60vh]">
        <Loader2 className="h-8 w-8 animate-spin text-nx-primary" />
      </div>
    );
  }
